...
const USE_AI_SDK = false; // Start with false to preserve existing functionality
...
export class AIServiceAdapter {
  private aiSDKService: {
    ...
    generateTextStream: (params: AIRequestParams) => AsyncGenerator<{ title?: string; description?: string; error?: string; isComplete?: boolean }, void, unknown>;
    isConfigured: () => boolean;
    getAvailableProviders: () => string[];
  } | null = null;
  private legacyService: {
    generateText: (params: AIRequestParams & { apiKey?: string }) => Promise<GenerateResponse>;
    generateTextStream: (params: AIRequestParams & { apiKey?: string }) => AsyncGenerator<StreamChunk, void, unknown>;
  } | null = null;
  ...
  private async getAIKeys() {
    try {
      ...
      return {
        openRouterKey: result.openRouterKey ? await decryptApiKey(result.openRouterKey) : null,
        openaiKey: result.openaiKey ? await decryptApiKey(result.openaiKey) : null,
        anthropicKey: result.anthropicKey ? await decryptApiKey(result.anthropicKey) : null,
        googleKey: result.googleKey ? await decryptApiKey(result.googleKey) : null,
        groqKey: result.groqKey ? await decryptApiKey(result.groqKey) : null,
        cerebrasKey: result.cerebrasKey ? await decryptApiKey(result.cerebrasKey) : null,
      };
    } catch (error) {
  ...
  private shouldUseAISDK(model: AIModel, keys: Record<string, string | null>): boolean {
  ...
  private getAIKeyForProvider(provider: string, keys: Record<string, string | null>): string | null {
    switch (provider) {
      case 'openai':
  ...
  private async getLegacyService() {
    if (!this.legacyService) {
      ...
      this.legacyService = {
        generateText: async () => {
          throw new Error('Legacy service not implemented in adapter - use background script functions');
        },
        generateTextStream: async function* () {
          yield { type: 'error', error: 'Legacy service not implemented in adapter' };
        },
      };
    }
  ...
  async generateText(params: AIRequestParams): Promise<GenerateResponse> {
    ...
    if (this.shouldUseAISDK(params.model, keys)) {
      ...
      return await legacy.generateText({
        ...params,
        apiKey: this.getAIKeyForProvider(params.model.provider || 'openrouter', keys) || undefined,
      });
      }
    } else {
      // Use legacy service
  ...
  async* generateTextStream(params: AIRequestParams): AsyncGenerator<StreamChunk> {
    const keys = await this.initializeAIKeys();
  ...
  setUseAISDK(enabled: boolean) {
    ...
    (globalThis as { AI_SERVICE_ADAPTER_FLAG?: boolean }).AI_SERVICE_ADAPTER_FLAG = enabled;
  }
